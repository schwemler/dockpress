	...
	var usof_colpick = function() {
		var
			tpl = '<div class="usof_colpick">' +
				...
				// '<div class="usof_colpick_new_color"></div>' +
				// '<div class="usof_colpick_current_color"></div>' +
			...
			downIncrement = function( ev ) {
				...
				var current = {
					el: $( this ).parent().addClass( 'usof_colpick_slider' ),
					max: this.parentNode.className.indexOf( '_hsb_h' ) > 0 ? 360 : ( this.parentNode.className.indexOf( '_hsb' ) > 0 ? 100 : 255 ),
					y: ev.pageY,
					field: field,
					val: parseInt( field.val(), 10 ),
					preview: $( this ).parent().parent().data( 'usof_colpick' ).livePreview
				};
			...
			show = function( ev ) {
				...
				// Check whether usof_colpick is shown
				if ( this.isShown && ! this.isFocus ) {
					if ( cal.data( 'usof_colpick' ).onHide.apply( this, [ cal.get( 0 ) ] ) != false ) {
						this.isFocus = false;
						this.isShown = false;
					}
				} else {
					if ( ( cal.data( 'usof_colpick' ).onShow.apply( this, [ cal.get( 0 ) ] ) != false ) && ! this.isFocus ) {
						this.isShown = true;
						this.isFocus = false;
					}
				}
				if ( this.isFocus && ! this.isShown ) {
					if ( cal.data( 'usof_colpick' ).onShow.apply( this, [ cal.get( 0 ) ] ) != false ) {
						this.isShown = true;
						this.isFocus = false;
					}
				}
				// Hide usof_colpick on click outside for not focus events
				if ( arguments[ 1 ] != 'focus' && arguments[ 1 ] ) {
					$( 'html' ).on( 'mousedown', function( ev ) {
						if ( cal.data( 'usof_colpick' ).onHide.apply( that, [ cal.get( 0 ) ] ) != false ) {
							that.isShown = false;
							that.isFocus = false;
						}
					} );
			...
			restoreOriginal = function() {
				var cal = $( this ).parent();
				var col = cal.data( 'usof_colpick' ).origColor;
				cal.data( 'usof_colpick' ).color = col;
				fillRGBAFields( col, cal.get( 0 ) );
				fillHexFields( col, cal.get( 0 ) );
				fillHSBAFields( col, cal.get( 0 ) );

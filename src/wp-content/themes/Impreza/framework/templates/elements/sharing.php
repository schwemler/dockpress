...
$sharing_list = '';
foreach ( $providers_list as $provider => $provider_data ) {
	$sharing_list .= '<a class="w-sharing-item ' . $provider . '" title="' . esc_attr( $provider_data['title'] ) . '" href="javascript:void(0) ">';
...
if ( $text_selection ) {
	$output .= '<div class="w-sharing-tooltip" style="display: none">';
	$output .= '<div class="w-sharing-list" data-sharing-url="' . esc_attr( $url ) . '" data-sharing-image="' . esc_attr( $post_thumbnail ) . '">';
	$output .= $sharing_list;
	$output .= '<a class="w-sharing-item copy2clipboard " title="' . us_translate( 'Copy' ) . '" href="javascript:void(0)"><span class="w-sharing-icon"></span></a>';
	$output .= '</div></div>';
}

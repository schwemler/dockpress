...
if ( $color_scheme != '' ) {
	...
	if ( $color_scheme == 'custom' ) {
		// Custom colors
		if ( $us_bg_color != '' ) {
			$inline_css .= 'background-color: ' . $us_bg_color . ';';
		}
		if ( $us_text_color != '' ) {
...
if ( ! empty( $us_bg_overlay_color ) ) {
	...
	$bg_overlay_html = '<div class="l-section-overlay" style="background-color: ' . $us_bg_overlay_color . '"></div>';
}
...
if ( substr( $inner_output, 0, 18 ) != '<div class="g-cols' ) {
	$cols_class_name = ' type_' . $columns_type;
	if ( ! empty( $content_placement ) ) {
		$cols_class_name .= ' valign_' . $content_placement;
	}
	if ( ! empty( $gap ) ) {
		$cols_class_name .= ' vc_column-gap-' . $gap;
	}

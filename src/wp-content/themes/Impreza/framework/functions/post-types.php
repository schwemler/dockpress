		...
		if ( $column_name == 'used_in' ) {
			...
			foreach ( $pages_layout_types as $type => $title ) {
				...
				if ( isset( $usof_options['titlebar' . $type . '_id'] ) AND $usof_options['titlebar' . $type . '_id'] == $post_ID ) {
					$used_in['pages_layout'][] = $title . ' > ' . __( 'Title Bar', 'us' );
				}
				if ( isset( $usof_options['content' . $type . '_id'] ) AND $usof_options['content' . $type . '_id'] == $post_ID ) {
					$used_in['pages_layout'][] = $title . ' > ' . us_translate( 'Content' );
				}
				if ( isset( $usof_options['footer' . $type . '_id'] ) AND $usof_options['footer' . $type . '_id'] == $post_ID ) {
					$used_in['pages_layout'][] = $title . ' > ' . __( 'Footer', 'us' );
				}
			}
			...
			$archives_layout_types = array_merge(
				array(
					'_archive' => us_translate( 'Archives' ),
					'_author' => __( 'Authors', 'us' ),
				), us_get_taxonomies( TRUE, FALSE )
			);

			foreach ( $archives_layout_types as $type => $title ) {
				$type = ( $type != '_archive' AND $type != '_author'  ) ? ( '_tax_' . $type ) : $type;

				if ( isset( $usof_options['titlebar' . $type . '_id'] ) AND $usof_options['titlebar' . $type . '_id'] == $post_ID ) {
					$used_in['archives_layout'][] = $title . ' > ' . __( 'Title Bar', 'us' );
				}
				if ( isset( $usof_options['content' . $type . '_id'] ) AND $usof_options['content' . $type . '_id'] == $post_ID ) {
					$used_in['archives_layout'][] = $title . ' > ' . us_translate( 'Content' );
				}
				if ( isset( $usof_options['footer' . $type . '_id'] ) AND $usof_options['footer' . $type . '_id'] == $post_ID ) {
					$used_in['archives_layout'][] = $title . ' > ' . __( 'Footer', 'us' );
				}
			}
			...
			foreach ( $wpdb->get_results( $usage_query ) as $usage_result ) {
				...
				if ( $post ) {
					$is_custom_area = get_post_meta( $post->ID, 'us_titlebar', TRUE ) == 'custom';
					if ( $is_custom_area ) {
						$used_in['meta_options'][$post->ID] = array(
			...
			$usage_query = "SELECT post_id FROM {$wpdb->prefix}postmeta WHERE meta_key = 'us_content_id' AND meta_value = '" . $post_ID . "' LIMIT 0, 100";
			foreach ( $wpdb->get_results( $usage_query ) as $usage_result ) {
				$post = get_post( $usage_result->post_id );
				if ( $post ) {
					$is_custom_area = get_post_meta( $post->ID, 'us_content', TRUE ) == 'custom';
					if ( $is_custom_area ) {
						$used_in['meta_options'][$post->ID] = array(
							'url' => get_permalink( $post->ID ),
							'title' => get_the_title( $post->ID ),
			...
			foreach ( $wpdb->get_results( $usage_query ) as $usage_result ) {
				$post = get_post( $usage_result->post_id );
				if ( $post ) {
					$is_custom_area = get_post_meta( $post->ID, 'us_footer', TRUE ) == 'custom';
					if ( $is_custom_area ) {
						$used_in['meta_options'][$post->ID] = array(
							'url' => get_permalink( $post->ID ),
							'title' => get_the_title( $post->ID ),
			...
			// Output the result
			foreach ( $used_in['pages_layout'] as $where ) {
				echo '<strong>' . $where . '</strong> (<a href="' . admin_url() . 'admin.php?page=us-theme-options#pages_layout">' . __( 'edit in Theme Options', 'us' ) . '</a>)<br>';
			}
			foreach ( $used_in['archives_layout'] as $where ) {
				echo '<strong>' . $where . '</strong> (<a href="' . admin_url() . 'admin.php?page=us-theme-options#archives_layout">' . __( 'edit in Theme Options', 'us' ) . '</a>)<br>';
			}
			foreach ( $used_in['meta_options'] as $where ) {

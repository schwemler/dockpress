...
class US_Layout {
	...
	protected function __construct() {
		...
		if ( is_singular( array( 'us_portfolio' ) ) ) {
			...
			if ( us_get_option( 'sidebar_author_id' ) == '' ) {
				$this->sidebar_pos = 'none';
			} elseif ( us_get_option( 'sidebar_author_id' ) != '__defaults__' ) {
				$this->sidebar_pos = us_get_option( 'sidebar_author_pos' );
			} elseif ( us_get_option( 'sidebar_archive_id' ) != '' ) {
			...
			if ( ! empty( $current_tax ) AND in_array( $current_tax, $public_taxonomies ) ) {
				if ( us_get_option( 'sidebar_tax_' . $current_tax . '_id' ) == '' ) {
					$this->sidebar_pos = 'none';
				} elseif ( us_get_option( 'sidebar_tax_' . $current_tax . '_id' ) != '__defaults__' ) {
					$this->sidebar_pos = us_get_option( 'sidebar_tax_' . $current_tax . '_pos' );
				}
			}
		...
		if ( is_singular(
			...
			) OR ( ( is_404() OR is_search() OR is_home() ) AND $postID != NULL ) ) {

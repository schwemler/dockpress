		...
		switch ( $prop ) {
			...
			case 'font-family':
				...
				if ( in_array( $tag, array( 'h1', 'h2', 'h3', 'h4', 'h5', 'h6' ) ) ) {
					if ( $tag == $value ) {
						continue;
					} elseif ( in_array( $value, array( 'h1', 'h2', 'h3', 'h4', 'h5', 'h6' ) ) AND us_get_option( $tag . '_font_family' ) == 'get_h1|' ) {
						continue;
					} else {
 ...
 *
 * @param string $area : header / titlebar / content / sidebar / footer
 *
 * @return string
 */
function us_get_page_area_id( $area ) {
	...
	// Search Results page
	if ( is_search() AND $postID = us_get_option( 'search_page' ) ) {
	...
	// Posts page
	if ( is_home() AND $postID = us_get_option( 'posts_page' ) ) {
		if ( usof_meta( 'us_' . $area, array(), $postID ) == 'hide' ) {
			$area_id = '';
		}
		if ( usof_meta( 'us_' . $area, array(), $postID ) == 'custom' ) {
	...
	// 404 page
	if ( is_404() AND $postID = us_get_option( 'page_404' ) ) {
		if ( usof_meta( 'us_' . $area, array(), $postID ) == 'hide' ) {
			$area_id = '';
		}
		if ( usof_meta( 'us_' . $area, array(), $postID ) == 'custom' ) {
 ...
 *
 * @param mixed $link
 * @param string $us_elm_context : shortcode / grid / header / widget
 *
 * @return string
 */
function us_generate_link_atts( $link, $us_elm_context ) {

	if ( ! isset( $link ) OR ! isset( $us_elm_context ) ) {
		return '';
	}
	$link_array = array();
	...
	if ( $us_elm_context == 'shortcode' ) {
		...
		$link_array['href'] = $link_array['url']; // 'href' attribute needed instead of 'url'

		// Header & Grid
	} else {
		$custom_link_atts = usof_get_link_atts( $link ); // TODO: merge this function to here

		$link_array['href'] = ( ! empty( $custom_link_atts['href'] ) ) ? $custom_link_atts['href'] : '';
		$link_array['meta'] = ( ! empty( $custom_link_atts['target'] ) ) ? ' target="' . esc_attr( $custom_link_atts['target'] ) . '"' : '';

		// Get link URL from custom field when it set as {{field_name}}
		if ( preg_match( "#^{{([^}]+)}}$#", trim( $link_array['href'] ), $matches ) ) {
				...
				if ( substr( strval( $meta_value ), 0, 1 ) === '{' ) {
					try {
						...
						if ( is_array( $meta_value_array ) ) {
							...
							$link_array['href'] = ( ! empty( $meta_link_atts['href'] ) ) ? $meta_link_atts['href'] : '';
							$link_array['meta'] .= ( ! empty( $meta_link_atts['target'] ) AND strpos( $link_array['meta'], 'target=' ) === FALSE ) ? ' target="' . esc_attr( $meta_link_atts['target'] ) . '"' : '';
							$link_array['meta'] .= ' rel="nofollow"'; // force "nofollow" for metabox URLs
						}
		...
		if ( ! empty( $link_array['href'] ) AND strpos( $link_array['href'], '[lang]' ) !== FALSE ) {
			$link_array['href'] = str_replace( '[lang]', usof_get_lang(), $link_array['href'] );
		}
	}
	...
	if ( isset( $link_array['href'] ) AND $link_array['href'] != '' ) {
		$result = ' href="' . esc_url( $link_array['href'] ) . '"';
		$result .= ( ! empty( $link_array['title'] ) ) ? ( ' title="' . esc_attr( $link_array['title'] ) . '"' ) : '';
		$result .= ( ! empty( $link_array['target'] ) ) ? ( ' target="' . esc_attr( $link_array['target'] ) . '"' ) : '';
		$result .= ( ! empty( $link_array['rel'] ) ) ? ( ' rel="' . esc_attr( $link_array['rel'] ) . '"' ) : '';
		$result .= ( ! empty( $link_array['meta'] ) ) ? $link_array['meta'] : '';
	} else {
		$result = '';
	}

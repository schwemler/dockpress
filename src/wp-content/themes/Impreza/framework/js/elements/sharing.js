...
! function( $ ) {
	...
	function WShare( selector ) {
		...
		if ( $this.hasClass( 'facebook' ) ) {
			window.open( "https://www.facebook.com/sharer/sharer.php?u=" + encodeURIComponent( opt.url ) + "&quote=" + encodeURIComponent( opt.text ) + "", "", "toolbar=0, status=0, width=900, height=500" );
			...
			if ( jQuery.isMobile ) {
				window.open( "https://api.whatsapp.com/send?text=" + encodeURIComponent( opt.text + ' ' + opt.url ), "", "toolbar=0, status=0, width=900, height=500" );
			} else {
				window.open( "https://web.whatsapp.com/send?text=" + encodeURIComponent( opt.text + ' ' + opt.url ), "", "toolbar=0, status=0, width=900, height=500" );
			}
		} else if ( $this.hasClass( 'xing' ) ) {
			window.open( "https://www.xing.com/spi/shares/new?url=" + encodeURIComponent( opt.url ), "", "toolbar=no, status=0, width=900, height=500" );
		} else if ( $this.hasClass( 'reddit' ) ) {
			window.open( "https://www.reddit.com/submit?url=" + encodeURIComponent( opt.url ) + "&title=" + encodeURIComponent( opt.text ), "", "toolbar=no, status=0, width=900, height=500" );
			...
			window.open( 'https://vk.com/share.php?url=' + encodeURIComponent( opt.url ) + '&title=' + encodeURIComponent( opt.text ), '&description=&image=' + encodeURIComponent( opt.image ), 'toolbar=no,width=700,height=300' );
		} else if ( $this.hasClass( 'email' ) ) {
			window.location = 'mailto:?subject=' + opt.text + '&body=' + opt.url;
		}
	}

	$( 'body' ).not( '.w-post-elm.post_content' ).bind( 'mouseup', function( e ) {
		var selection;
	...
	$( '.w-post-elm.post_content' ).bind( 'mouseup', function( e ) {
		//if Enable sharing via text selection

...
function washing_center_scripts() {
	...
	wp_enqueue_script( 'washing-center-skip-link-focus-fix', get_theme_file_uri( '/assets/js/skip-link-focus-fix.js' ), array(), '1.0', true );

	$washing_center_l10n=array();
	
	if ( has_nav_menu( 'top' ) ) {
		wp_enqueue_script( 'washing-center-navigation-jquery', get_theme_file_uri( '/assets/js/navigation.js' ), array( 'jquery' ), '1.0', true );
		$washing_center_l10n['expand']         = __( 'Expand child menu', 'washing-center' );
		$washing_center_l10n['collapse']       = __( 'Collapse child menu', 'washing-center' );
	}
	...
	wp_localize_script( 'washing-center-skip-link-focus-fix', 'washing_centerScreenReaderText', $washing_center_l10n );
...
//footer Link
define('WASHING_CENTER_CREDIT','https://www.luzuk.com/','washing-center');

if ( ! function_exists( 'washing_center_credit' ) ) {
	function washing_center_credit(){
		echo "<a href=".esc_url(WASHING_CENTER_CREDIT)." target='_blank'>".esc_html__('Luzuk','washing-center')."</a>";
	}
}
